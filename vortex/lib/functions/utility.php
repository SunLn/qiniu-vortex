...
function vortex_post_author() {
	...
	$output = sprintf( '<span class="entry-author author vcard"><a href="%1$s" title="'. __( 'by %2$s', 'vortex' ) .'" rel="author">%2$s</a></span>', esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ), esc_html( get_the_author() ) );
	return $output;
...
function vortex_post_edit_link() {
	...
	edit_post_link( __( '编辑', 'vortex' ), sprintf( '<span class="edit-link">'), '</span>' );
	else:
	edit_post_link( __( '编辑', 'vortex' ), '<span class="edit-link">', '</span>' );
	endif;
	$output = ob_get_clean();
...
function vortex_post_category() {
	...
	$output = sprintf( '<span class="cat-links"><span class="%1$s">'. __( '发布在', 'vortex' ) .'</span> %2$s</span>', 'entry-utility-prep entry-utility-prep-cat-links', $categories_list );
	return $output;
}
...
function vortex_post_tags() {
	...
	$output = sprintf( '%3$s<span class="tag-links"><span class="%1$s">'. __( '标签:', 'vortex' ) .'</span> %2$s</span>', 'entry-utility-prep entry-utility-prep-tag-links', $tags_list, vortex_entry_meta_sep() );
	return $output;
}
...
function vortex_featured_image() {
	...
	$img = vortex_get_image( array( 'format' => 'html', 'mode' => $vortex_options['vortex_featured_image_control'], 'attr' => array( 'class' => 'entry-featured-image' ) ) );
	if( empty( $img ) ) {		
		return;
	}
...
function vortex_loop_nav_next_prev() {
	...
	ob_start();
	next_posts_link( '<span class="meta-nav">&larr;</span> '. __( '更老的文章', 'vortex' ) );
	$next_posts_link = ob_get_clean();
	
	ob_start();
	previous_posts_link( __( '更新的文章', 'vortex' ) .' <span class="meta-nav">&rarr;</span>' );
...
function vortex_loop_nav_singular_post() {
	...
	previous_post_link( '%link', '<span class="meta-nav">&larr;</span> '. __( '%title', 'vortex' ) );
	$previous_post_link = ob_get_clean();
	  
	ob_start();
	next_post_link( '%link', __( '%title', 'vortex' ) . ' <span class="meta-nav">&rarr;</span>' );
...
<div class="grid_11">
  <?php printf( __( '%s Theme by', 'vortex' ), $vortex_theme_data['Name'] ); ?> <a href="http://wordpress.org/themes/vortex" title="WPVortex">WPVortex</a> &sdot;<a href="http://wordpress.org/" title="WordPress">WordPress</a>&sdot;Redesigned by <a href="http://www.qiniu.com" title="Qiniu.com">Qiniu</a>
</div>
<?php	
}
...
function vortex_content_more_link( $more_link, $more_link_text ) {
	return str_replace( $more_link_text, '<span>'. __( '继续阅读…', 'vortex' ) .'</span>', $more_link );
}

/** Returns a "Read more" link for excerpts */
function vortex_continue_reading_link() {
	return '<span class="more-link-wrap"><a href="'. esc_url( get_permalink() ) . '" class="more-link"><span>'. __( '继续阅读…', 'vortex' ) .'</span></a></span>';
}
